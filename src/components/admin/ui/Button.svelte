<script lang="ts">
  ...
  const baseClasses = "inline-flex items-center justify-center font-semibold transition-all focus:outline-none";
  ...
  const disabledClasses = "opacity-50 cursor-not-allowed hover:-translate-y-0 shadow-none pointer-events-none";

  let computedClasses = $derived(
    [baseClasses, variantClasses[variant], sizeClasses[size], disabled ? disabledClasses : "cursor-pointer", className]
      .filter(Boolean)
      .join(" "),
  );
</script>
