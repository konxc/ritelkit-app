<script lang="ts">
  ...
  const handleUpdateStatus = async (event: SubmitEvent) => {
    ...
    try {
      ...
      toastRef?.show(err.message || "Update failed", "error");
    } finally {
      isSubmitting = false;
    }
  };
...
<div class="flex flex-col gap-8 pb-12 print:p-0">
  <!-- Header / Breadcrumb -->
  <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between no-print">
    <div class="flex items-center gap-4">
      ...
      <div>
        <div class="flex items-center gap-2">
          <h1 class="text-2xl font-black tracking-tight text-stone-900 uppercase font-mono">{order.orderNo}</h1>
        ...
        <div class="mt-1 flex items-center gap-2 text-xs font-bold text-stone-400 uppercase tracking-widest">
           <span>{new Date(order.createdAt).toLocaleDateString(t("common.lang_code"), { day: 'numeric', month: 'long', year: 'numeric' })}</span>
           <span class="h-1 w-1 rounded-full bg-stone-300"></span>
           <span>{new Date(order.createdAt).toLocaleTimeString(t("common.lang_code"), { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3">
       <Button variant="outline" onclick={handlePrint} class="h-11 rounded-2xl px-6 font-bold text-stone-600">
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect width="12" height="8" x="6" y="14"/></svg>
          {t("common.export") || "Print"}
       </Button>
       <Button variant="primary" href={`/admin/fulfillment?q=${order.orderNo}`} class="h-11 rounded-2xl px-6 font-black tracking-wide uppercase">
  ...
  <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4" in:fly={{ y: 20, duration: 400 }}>
    <PanelCard class="relative overflow-hidden border-stone-200/60 bg-white shadow-sm">
      <div class="flex items-center gap-3">
        ...
        <div class="min-w-0">
          ...
          {#if order.customerId}
            <a href={`/admin/customers/${order.customerId}`} class="block truncate text-sm font-black text-[#c48a3a] hover:underline">
              {order.customerName}
            </a>
          {:else}
    ...
    <PanelCard class="relative overflow-hidden border-stone-200/60 bg-white shadow-sm">
      ...
      <div class="mt-3 pl-13">
         <Badge variant="default" class="text-[0.6rem] py-0.5">
  ...
  <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 flex flex-col gap-6">
      <PanelCard class="border-stone-200/60 bg-white p-0 shadow-sm overflow-hidden">
            ...
            {#each order.parsedItems || [] as item, i (item.name + i)}
              <TableRow class="group transition-colors hover:bg-stone-50/30">
                ...
                <TableCell class="py-5 text-center">
                  <span class="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-stone-100 font-mono text-xs font-black text-stone-600">
                    {item.qty}
                  </span>
                </TableCell>
                <TableCell class="py-5 text-right font-mono text-sm text-stone-500 tabular-nums">
                  {t("common.currency_symbol")} {Number(item.price).toLocaleString(t("common.lang_code"))}
                </TableCell>
                <TableCell class="py-5 text-right font-mono text-[0.9rem] font-black text-stone-900 tabular-nums text-[#c48a3a]">
                  {t("common.currency_symbol")} {Number(item.total).toLocaleString(t("common.lang_code"))}
                </TableCell>
              </TableRow>
            {/each}
          </Table>
        ...
        <div class="mt-4 space-y-4 bg-stone-50/50 p-6 lg:p-8">
          <div class="flex justify-between text-sm font-bold text-stone-500">
            <span class="uppercase tracking-wider">{t("orders.subtotal")}</span>
          ...
          {#if order.discountTotal > 0}
            <div class="flex justify-between text-sm font-bold text-rose-500">
              <span class="uppercase tracking-wider">{t("orders.discount")} {order.couponCode ? `(${order.couponCode})` : ""}</span>
              <span class="font-mono tabular-nums"
          ...
          {/if}
          <div class="flex justify-between text-sm font-bold text-stone-500">
            <span class="uppercase tracking-wider">{t("orders.shipping_fee")}</span>
            <span class="font-mono tabular-nums"
              >{t("common.currency_symbol")}
          ...
          <div class="flex justify-between border-t border-stone-200 pt-5">
            <span class="text-sm font-black uppercase tracking-widest text-stone-900">{t("orders.total")}</span>
            <div class="flex items-baseline gap-1">
      ...
      <PanelCard class="border-stone-200/60 bg-white p-0 shadow-sm overflow-hidden no-print">
        <div class="border-b border-stone-100 bg-stone-50/50 px-6 py-4">
           <h3 class="text-[0.75rem] font-black tracking-widest text-stone-500 uppercase">{t("orders.order_flow") || "Riwayat Status"}</h3>
        </div>
        <div class="p-6">
           <div class="flex flex-col gap-6 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-stone-100">
              {#each order.statusHistory as history}
                <div class="flex gap-4 relative">
                   <div class="h-6 w-6 rounded-full border-4 border-white ring-2 ring-stone-100 shrink-0 z-10 {history.status === order.status ? 'bg-[#c48a3a]' : 'bg-stone-200'}"></div>
                   <div class="flex flex-col gap-1 min-w-0">
                      <div class="flex items-center gap-3">
                         <span class="font-black text-sm text-stone-900 uppercase tracking-tighter">
                           {t(`orders.statuses.${history.status?.toLowerCase()}`) || history.status}
                         </span>
                         <span class="text-[0.65rem] font-bold text-stone-400">
                           {new Date(history.createdAt).toLocaleString(t("common.lang_code"), { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                         </span>
                      </div>
                      {#if history.notes}
                        <p class="text-xs text-stone-500 italic leading-relaxed border-l-2 border-stone-100 pl-3">
                          {history.notes}
                        </p>
                      {/if}
                   </div>
    ...
    <div class="flex flex-col gap-6 no-print">
      <PanelCard class="border-stone-200/60 bg-white shadow-sm">
        <div class="mb-4 flex items-center justify-between border-b border-stone-100 pb-3">
           ...
           <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-stone-300"><path d="M10 17h4V5H2v12h3"/><path d="M20 17h2v-3.34a4 4 0 0 0-1.17-2.83L19 9h-5"/><circle cx="7" cy="17" r="2"/><circle cx="17" cy="17" r="2"/></svg>
        </div>
        <div class="space-y-5 text-sm">
          ...
          <div>
            <span class="text-[0.6rem] font-bold tracking-widest text-stone-400 uppercase">{t("orders.full_address")}</span>
          ...
          {#if order.shippingAddress?.notes}
            <div class="rounded-2xl border border-amber-100 bg-amber-50/50 p-4">
              <span class="text-[0.6rem] font-black text-amber-600 uppercase tracking-wider">{t("orders.buyer_notes")}</span>
              <p class="mt-1 text-xs font-semibold text-amber-900 italic leading-relaxed">"{order.shippingAddress.notes}"</p>
            </div>
          {/if}
        </div>
      </PanelCard>
      ...
      <PanelCard class="border-stone-200/60 bg-white shadow-sm">
        <div class="mb-5 flex items-center justify-between border-b border-stone-100 pb-3">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="text-stone-300"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>
        </div>
        <CrudInlineForm id="order-status-form" onsubmit={handleUpdateStatus} {isSubmitting}>
          <div class="flex flex-col gap-6">
            ...
            />
            <Button type="submit" variant="primary" class="h-12 w-full font-black tracking-widest uppercase shadow-lg shadow-[#c48a3a]/20" disabled={isSubmitting}>
              {#if isSubmitting}
            ...
            <div class="mt-2 grid grid-cols-2 gap-3">
              <Button variant="outline" class="h-11 rounded-xl text-[0.65rem] font-bold uppercase tracking-wider" href="/admin/orders">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mr-1.5"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
                {t("common.delete") || "Hapus"}
              </Button>
              <Button variant="outline" class="h-11 rounded-xl text-[0.65rem] font-bold uppercase tracking-wider text-rose-500 border-rose-100 hover:bg-rose-50" href="/admin/refunds">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" class="mr-1.5"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg>
                {t("orders.refund")}
              </Button>
            </div>
          </div>
