  ...
  const handleRowAction = (id: string, action: string, rowElement: HTMLElement | null) => {
    if (action === "delete") {
      ...
      const data = {
        ...
        maxDiscount: fields.max_discount === "∞" || !fields.max_discount ? null : Number(fields.max_discount),
        startAt: fields.start_at === "-" || !fields.start_at ? null : fields.start_at,
        endAt: fields.end_at === "-" || !fields.end_at ? null : fields.end_at,
        usageLimit: fields.usage_limit === "∞" || !fields.usage_limit ? null : Number(fields.usage_limit),
