...
export async function getDashboardStats(db: Db) {
	...
	const pendingShipments = await db
		...
		.where(
			or(
				eq(schema.shipments.status, "packing"),
				eq(schema.shipments.status, "shipped"),
			),
		)
		.get();
...
export async function getReportData(db: Db) {
	...
	const ordersRes = await db
		.select()
		.from(orders)
		.where(gte(orders.createdAt, thirtyDaysAgoStr));
	...
	const refundsRes = await db
		...
		.where(
			and(
				...
				gte(refunds.createdAt, thirtyDaysAgoStr),
			),
		);
	...
	// LTV (90 days)
	const paidOrders90 = orders90Res.filter(
		(row: any) => row.paymentStatus === "paid",
	);
	const uniqueCustomers = new Set(
		paidOrders90.map((row: any) => row.customerPhone),
	);
	...
	const adsRes = await db
		...
		.where(
			or(
				eq(adsCampaigns.status, "active"),
				eq(adsCampaigns.status, "completed"),
			),
		);

	const totalSpend = adsRes.reduce(
