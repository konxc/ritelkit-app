...
async function ensureAdminUsers(db: Client, now: string) {
  ...
  if (Number((count.rows[0] as any)?.count || 0) > 1) return; // Allow 1 default admin
...
async function ensureSettings(db: Client, now: string) {
  ...
  await db.execute({
    ...
    args: [
      "order_settings",
      JSON.stringify({ preorder_only: false, same_day_enabled: true, minimum_lead_time_hours: 2 }),
      now,
    ],
  });
  await db.execute({
    ...
    args: [
      "delivery_settings",
      JSON.stringify({ delivery_province: "DI Yogyakarta", free_delivery_threshold: 150000 }),
      now,
    ],
  });
}
...
async function ensureOrders(
  ...
  if (Number((count.rows[0] as any)?.count || 0) > 5) return; // Already seeded enough
  ...
  for (let i = 0; i < 50; i++) {
    ...
    for (let j = 0; j < itemNum; j++) {
      randomItems.push({ slug: productSlugs[Math.floor(Math.random() * productSlugs.length)], qty: Math.floor(Math.random() * 3) + 1 });
    }
...
async function ensureShipments(db: Client, orderMap: Record<string, { id: string; order_no: string }>, now: string) {
...
async function ensureOrderStatusHistory(db: Client, orderMap: Record<string, { id: string; order_no: string; created_at: string }>) {
...
async function ensureNotifications(db: Client, now: string, orderMap: Record<string, { order_no: string; customer_email: string; customer_phone: string }>) {
  ...
  for (const fixture of notificationFixtures) {
    ...
    const recipient =
      fixture.recipientField === "customer_email"
...
async function ensureAuditLogs(
  ...
  if (Number((count.rows[0] as any)?.count || 0) > 0) return;
  const references = [
    { entity: "order", id: orderMap["RS-20260226-001"]?.id, note: "Sample order entry" },
    { entity: "product", id: productMap["roti-coklat-sholawat"]?.id, note: "Seed product" },
  ];
...
async function buildOrderMap(db: Client) {
  ...
  const map: Record<string, { id: string; order_no: string; created_at: string; customer_email: string; customer_phone: string }> = {};
  (rows.rows as any[]).forEach((row) => {
    if (row.order_no) {
