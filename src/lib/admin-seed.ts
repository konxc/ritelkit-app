...
const sampleProducts: SampleProduct[] = [
	{
		...
		sku: "RS-CHOC",
		description:
			"Roti isi cokelat premium dengan tekstur lembut khas Roti Sholawat.",
	...
	},
	{
		...
		sku: "RS-TAWAR",
		description:
			"Roti tawar gandum sehat, tanpa pengawet, cocok untuk sarapan keluarga.",
...
async function ensureAdminUsers(db: Client, now: string) {
	...
	if (Number((count.rows[0] as { count?: number } | undefined)?.count || 0) > 1)
		return; // Allow 1 default admin
...
async function ensureProductReviews(
	db: Client,
	...
	if (Number((count.rows[0] as { count?: number } | undefined)?.count || 0) > 0)
		return;
...
async function ensureShippingRules(db: Client, now: string) {
	const count = await db.execute(
		"SELECT COUNT(*) as count FROM shipping_rules",
	);
	if (Number((count.rows[0] as { count?: number } | undefined)?.count || 0) > 0)
		return;
...
async function ensureOrders(
	...
	productMap: Record<
		string,
		{ id: string; price: number; slug: string; name: string }
	>,
) {
	const count = await db.execute("SELECT COUNT(*) as count FROM orders");
	if (Number((count.rows[0] as { count?: number } | undefined)?.count || 0) > 5)
	...
	for (let i = 0; i < 50; i++) {
		...
		const fixture: OrderFixture = {
			...
			delivery_fee:
				city === "Yogya" ? 0 : 15000 + Math.floor(Math.random() * 20000),
...
async function insertOrderFixture(
	...
	const items = fixture.items
		...
		.filter(
			(item): item is { product_id: string; qty: number; price: number } =>
				Boolean(item),
		);
...
async function ensureShipments(
	...
	for (const fixture of shipmentFixtures) {
		...
		const shippedAt =
			fixture.shippedOffset !== null ? isoDaysAgo(fixture.shippedOffset) : null;
		const deliveredAt =
			fixture.deliveredOffset !== null
				? isoDaysAgo(fixture.deliveredOffset)
				: null;
		await db.execute({
...
async function ensureOrderStatusHistory(
	...
	orderMap: Record<
		string,
		{ id: string; order_no: string; created_at: string }
	>,
) {
	const count = await db.execute(
...
async function ensureNotifications(
	...
	orderMap: Record<
		string,
		{ order_no: string; customer_email: string; customer_phone: string }
	>,
) {
	...
	for (const fixture of notificationFixtures) {
		...
		const sentAt =
			fixture.sentOffset !== null ? isoDaysAgo(fixture.sentOffset) : null;
		const recipient =
...
async function buildCouponMap(db: Client) {
	const rows = await db.execute(
		"SELECT id, code, type, value, max_discount FROM coupons",
	);
	const map: Record<
		string,
		{ id: string; type: string; value: number; max_discount: number | null }
