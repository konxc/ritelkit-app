...
export async function POST(ctx: APIContext) {
  ...
  const signature = await crypto.subtle.digest("SHA-512", new TextEncoder().encode(data));
  ...
  await db.execute({
    ...
    args: [crypto.randomUUID(), orderId, orderStatus, `Midtrans: ${transactionStatus}`, nowIso()],
  });
