...
export async function POST(ctx: APIContext) {
    ...
    const name = sanitizeText(String((body as any).name || ""));
    const channel = sanitizeText(String((body as any).channel || ""));
    const budget = asInt((body as any).budget, 0);
    const status = sanitizeText(String((body as any).status || "draft"));
    const startAt = sanitizeText(String((body as any).start_at || ""));
    const endAt = sanitizeText(String((body as any).end_at || ""));
    const notes = sanitizeText(String((body as any).notes || ""));
    if (!name || !channel || budget <= 0) {
        return new Response("Nama, channel, dan budget wajib diisi", { status: 400 });
    }
    const now = nowIso();
    const db = getDb(ctx);
