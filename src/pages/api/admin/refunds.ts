...
export async function POST(ctx: APIContext) {
    ...
    const orderNo = sanitizeText(String((body as any).order_no || ""));
    const amount = asInt((body as any).amount, 0);
    const status = sanitizeText(String((body as any).status || "requested"));
    const reason = sanitizeText(String((body as any).reason || ""));
    if (!orderNo || amount <= 0) {
    ...
    if (status === "approved") {
        ...
        const midtransOrderId = String((orderRes.rows[0] as any)?.midtrans_order_id || "");
        if (midtransOrderId) {
            ...
            try {
                ...
                await db.execute({
                    sql: "UPDATE refunds SET provider_status = ?, provider_response_json = ?, updated_at = ? WHERE id = ?",
                    args: ["error", String(err?.message || err), nowIso(), id],
                });
            }
        }
    }
    await logAudit(ctx, "create", "refund", id, { order_no: orderNo, amount, status });
