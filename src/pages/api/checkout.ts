...
import { readBody, json, getRequestId } from "../../lib/api";
import { getDb, initDb } from "../../lib/db";
import { getEnv, isProduction } from "../../lib/env";
import { calculateShipping, applyCoupon } from "../../lib/checkout";
...
export async function POST(ctx: APIContext) {
    ...
    const body: any = await readBody(ctx);
    ...
    const address: any = body.shipping_address || {};
    ...
    if (deliveryDate) {
        ...
        const requested = Date.parse(
            `${deliveryDate}T${deliveryTime || "08:00"}:00`,
        );
        if (!Number.isNaN(requested) && requested < minTime) {
            return new Response(
                "Waktu pengiriman tidak memenuhi minimum lead time",
