...
async function run() {
  ...
  for (const slug of pages) {
    ...
    await pageDesktop.screenshot({ path: path.join(outDir, `${slug}-desktop.png`), fullPage: true });
    ...
    await pageMobile.screenshot({ path: path.join(outDir, `${slug}-mobile.png`), fullPage: true });
